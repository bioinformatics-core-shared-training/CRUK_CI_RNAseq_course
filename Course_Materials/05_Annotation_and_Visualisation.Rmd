...
>
> (a) Remove the genes that we excluded by the independent filtering process of DESeq2
>
> (b) Create a column of -log10(FDR) values
>
> (c) Create a plot with points coloured by if FDR < 0.05
>
> (d) Limit the values at the top in order to see the lower portion more clearly.
...
We can't plot the entire data set, let's just select the top 200 by FDR. We'll
...
```{r saveEnvironment}
save.image(file="Robjects/Annotation_and_Visualisation.RData")
```


# Additional Material
There is additional material for you to work through in the [Supplementary Materials](../Supplementary_Materials) directory. Details include using genomic ranges and some extra useful plots like the one below.
