...
# counts <- read_tsv("data/GSE60450_Lactation.featureCounts", comment="#")
# # Retrieve list of all genes tested:
# assayedGenes <- counts$Geneid
# # Derive list of DEGs by filtering on FDR:
# sigGenes <- resLvV$GeneID[resLvV$FDR < 0.05 & !is.na(resLvV$FDR)]
# # create genes vector for goseq
# genes <- as.integer(assayedGenes%in%sigGenes)
# names(genes) <- assayedGenes
# 
# rm(counts)

# or is it

genes <- as.integer(resLvV$FDR < 0.05 & !is.na(resLvV$FDR))
names(genes) <- resLvV$GeneID
...
```{r pwFunction}
pwf <- nullp(genes, "mm10", "ensGene", bias.data = resLvV$Length)
```
...
```{r}
#?goseq
go.resLvV <- goseq(pwf, "mm10","ensGene")
go.resLvV
...
for(goAccession in go.resLvV.bp$category[1:10]){
    print(GOTERM[[goAccession]])
    cat("-----------------------------------------\n\n")
}
...
which we do not have Entrez IDs
...
```{r}
plotEnrichment(pathways[["HALLMARK_ESTROGEN_RESPONSE_EARLY"]],
               ranks)
```
...
* ROAST estimates p-values by simulation, so the resLvV may change slightly between runs. More precise p-values can be obtained by increasing the number of rotations, albeit at the cost of increased computational time.
...
You can write out the camera resLvV to a csv file to open in excel.
