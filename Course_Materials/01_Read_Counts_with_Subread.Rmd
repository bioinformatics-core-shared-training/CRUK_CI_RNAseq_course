...
found in the **`RNAseq/data`** directory under the **`Course_Materials`** 
...
We will use **`featureCounts`** programme from the [subRead
...
In fact we can have **`featureCounts`** do this for us by running all of our libraries together in one command. To save time for this we will use versions of our BAM files that only include alignments to chromosome 15. You can find these in **`smallBams`** directory.
