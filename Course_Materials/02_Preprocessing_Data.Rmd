...
The data for this tutorial comes from a Nature Cell Biology paper, [*EGF-mediated induction of Mcl-1 at the switch to lactation is essential for alveolar cell survival*](http://www.ncbi.nlm.nih.gov/pubmed/25730472) [@Fu2015]. Both the raw data (sequence reads) can be downloaded from SRA under [SRP045534](https://trace.ncbi.nlm.nih.gov/Traces/sra/sra.cgi?study=SRP045534)and processed data (counts) can be downloaded from Gene Expression Omnibus database (GEO) under accession number [GSE60450](http://www.ncbi.nlm.nih.gov/geo/query/acc.cgi?acc=GSE60450). Please see Supplementary material for instructions on downloading raw files, aligning fastq and generating a gene count matrix.
...
Let's create a counts matrix data object, `countdata`, that contains only the counts for the 12 samples.  
...
The rownames of the count matrix need to be the Gene IDs.
...
The column names of the count matrix need to be the sample names and need to
match the sample names in the `sampleinfo` table. Let's check this.
...
If the column names are not the same as in the sampleinfo table you can use 
various commands to alter the strings. It is better to do this in R rather than
editing the file in a spreadsheet as this is less likely to results in mistakes.

R has a lot of base functions for manipulating strings (text), e.g. `substr` 
and `paste` as shown in the chunk below. In addition the `stringr` package is 
worth investigating as it has many more useful commands.

```{r modifyColNames}
sample_name <- "SampleName.UnwantedSuffix"

# substr (subset string) can be used to extract a subset of the sample name
sample_name <- substr(sample_name, start=1, stop=10)
sample_name

# paste can be used to add to the sample name
sample_name <- paste(sample_name, "AdditionalSuffix", sep=".")
sample_name
```

If the samples are not in the same order you can use the `match` command to 
reorder the columns of the `countdata` or the rows of the `sampleinfo`. Use
`?match` to view the help document for this command.
...
# (1) load data
seq_c1 <- read.delim("<<YourCodeHere>>")

# (2) turn data into a counts matrix
counts_c1 <- seq_c1[,"<<YourCodeHere>>"]

# add rownames
"<<YourCodeHere>>"(counts_c1) <- seq_c1[,1]

# (3) calculate counts per million
cpm_c1 <- "<<YourCodeHere>>"(counts_c1)
```

> ## Additional Challenge 1a
> 1. Calculate the total counts for each sample and estimate the CPM cutoff equivalent to ~10 reads
> 1. For one sample, plot the Raw counts versus the CPM with a veritcal line showing
> your CPM cutoff
> 1. Add a horizontal line at 10 reads in a different colour (hint: use the `col` parameter)

**Solution**
```{r solutionChallenge1a, echo=FALSE}
...
glMDSPlot(dgeObj, labels=labels, groups=sampleinfo$Group, folder="mds")
```
