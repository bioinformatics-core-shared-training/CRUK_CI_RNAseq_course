...
- `librarySizes` is our data frame containing the variables we wish to plot
- `aes` creates a mapping between the variables in our data frame to the 
*aes*thetic proprties of the plot:
    + the x-axis is mapped to `Sample`
    + the y-axis is mapped to `Counts`
...
colour the points based on a p-value cut-off. The colours are automatically 
...
ggplot(logcounts, aes(x=Sample, y=logCounts, fill=Status)) +
        geom_boxplot() +
        geom_hline(yintercept = median(logcounts$logCounts), colour = "blue")
```
...
# # Check distributions of samples using boxplots
# # Add a horizontal line at the overall median 
# ggplot(rlogcounts, aes(x=Sample, y=logCounts, fill=Group)) +
#         geom_boxplot() +
#         geom_hline(yintercept = median(rlogcounts$logCounts), colour = "blue")
...
## PCA beyond the first two dimensions [optional]
