...
A principal components analysis (PCA) is an example of an unsupervised analysis,
...
`autoplot` plots the first two dimensions as a default, however you can plot
higher dimensions using the `x` and `y` arguments.
...
We don't want to plot a heatmap of all 22013 genes, so let's select data for
the 500 most variable genes and plot the heatmap.
