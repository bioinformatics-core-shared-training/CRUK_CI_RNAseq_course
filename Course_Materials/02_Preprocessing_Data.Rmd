...
colour to see if there is any difference between sample groups. We are going to
use the package ``ggplot2``[@Wickham2016] to create the plots.
...
`ggplot2` requires the data to be in "tidy" format. This is where each column
relates to a type of observation and each row to a single data point. In the
current format the `logcounts` matrix has rows that correspond to genes and 
columns that correspond to samples. We will need to transform the table so that
each row coresponds to a single count for a particular gene for a particular 
sample, this will have three columns - `GeneID`, `Sample`, `logCounts`.
