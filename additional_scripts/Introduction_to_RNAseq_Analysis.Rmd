...
dat %>% 
    ggplot(aes(x=Group, y=Expression, fill=Cat)) +
        geom_point(pch=21) +
        geom_abline(slope=ewe$coefficients[2], intercept = ewe$coefficients[1]) +
        expand_limits(y = 0) +
        scale_x_continuous(limits=c(0, 3), breaks=c(1, 2)) +
        guides(fill=FALSE) + theme(panel.background = element_blank())
