...
<img src="images/workflow.svg" class="centerimg" style="width: 70%;">
...
* Various factors mean the relationship between counts and RNA expression is not
the same for all genes across all samples
...
* Geometric mean scaling factor

* Assumes most genes are not differentially expressed

1. For each gene calculate the geometric mean across all samples
2. For each gene in each sample, normalise by dividing by the geometric mean for
that gene
3. For each sample calculate the scaling factor as the median of the normalised
counts

<div style="line-height: 50%;"><br></div>

<img src="images/geom_scaling.png" class="centerimg" style="width: 50%">
...
<div  class="centerimg" style="width: 75%">
```{r diffInMeans, echo=FALSE}
library(shape)
...
plot(x1, hx1, type="l", lty=1, xlab="x value",
...
<img src="images/Hamy2016PlosOneHC.png" style="width: 63%; 
                                               display: block;
                                               margin-left: 23%;">
