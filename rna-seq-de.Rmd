...
### Filtering
First, we will filter out very lowly expressed tags, keeping only genes that are expressed at a reasonable level in at least
one treatment condition.
```{r}
keep <- rowSums(cpm(dgeObj)>1) >= 6
dgeObj <- dgeObj[keep,]
dim(dgeObj)
```
...
### Normalisation
Now we compute effective library sizes using TMM normalization:
```{r}
...
```{r}
save(lrt,dgeObj,group,file="Robjects/DE.Rdata")
```
