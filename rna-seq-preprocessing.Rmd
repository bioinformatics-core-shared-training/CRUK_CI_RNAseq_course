...
> 1. Change the plotting characters such that basal samples have the value `1` and luminal samples have the value `4` and colour the points by status (lactate, pregnant, virgin)
>
...
```{r}
save(group,dgeObj,logcounts,sampleinfo,file="Robjects/preprocessing.Rdata")
```
