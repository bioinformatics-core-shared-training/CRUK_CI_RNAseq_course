...
glXYPlot(x=results$logFC, y=-log10(results$FDR),
         ...
         anno=ann.mod, id.column="ENTREZID", folder="volcano")
```
...
For instance, we can quickly identify overlapping regions between two `GenomicRanges`. However, we have to pay attention to the naming convention used for each object. `seqlevelsStyle` can help.
...
sigRegions <- keepSeqlevels(sigRegions, paste0("chr", c(1:19,"X","Y")), pruning.mode = "tidy")
```
...
my.reads <- readGAlignments(file="data/bam/MCL1.DG.bam",
...
bam <- readGAlignments(file="data/bam/MCL1.DG.bam",
