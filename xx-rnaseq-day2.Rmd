---
title: "COMBINE RNA-seq workshop Day 2"
author: ' Belinda Phipson, Maria Doyle '
date: "11-12 May 2016"
...
Data files: https://figshare.com/s/f5d63d8c265a05618137
...
Let's load the targets file that describes our data, and which contains the filenames of the files that we wish to map.

```{r}
targets <- read.delim("targets2.txt", stringsAsFactors=FALSE)
targets
```
...
We can specify the output files, or we can let `Rsubread` choose the output file names for us. The default output file name is the filename with ".subread.BAM" added at the end. Let's choose our own outfile names:

```{r}
# The sub command substitutes "fastq.gz" with "bam" in our filename
out.files <- sub(".fastq.gz", ".bam", targets$SRAFileName)
out.files
```
...
```{r,results="hide"}
align(index="chr1_mm10",readfile1=targets$SRAFileName,output_file=out.files)
```

This will align each of the 12 samples one after the other. As we're only using a subset of 1000 reads per sample, aligning should just take a minute or so for each sample. To run the full samples from this dataset would take about 2 hours per sample. The BAM files are saved in the working directory.
...
```{r}
props <- propmapped(files=out.files)
props
```

Exercise: (1) Try aligning the fastq files allowing multi-mapping reads (set `unique = FALSE`), and allowing for up to 6 "best" locations to be reported (`nBestLocations = 6`). Specify the output file names (out.files.multi) by substituting ".fastq.gz" with ".multi.bam" so we don't overwrite our unique alignment bam files (2) Look at the proportion of reads mapped and see if we get any more reads mapping by specifying a less stringent criteria. 
...
We can get the percentages of A, T, G and C in a fastq file by using the `atgcContent` function. This can sometimes highlight if there's a strong GC bias, for example.

```{r}
actg <- atgcContent(filename="SRR1552450.fastq.gz")
actg
```

Exercise: (1) Extract quality scores for SRR1552451.fastq.gz for 50 reads (2) Plot a boxplot of the quality scores for SRR1552451.fastq.gz. (3) Get the nucleotide percentages for SRR1552450.fastq.gz.
...
```{r,results="hide"}
fc <- featureCounts(out.files, annot.inbuilt="mm10")
```
...
Exercise: (1) Redo the counting over the exons, rather than the genes (specify `useMetaFeatures = FALSE`). Use the bam files generated doing alignment reporting only unique reads, and call the `featureCounts` object `fc.exon`. Check the dimension of the counts slot to see how much larger it is. (2) Using your ".multi.bam" files, redo the counting over genes, allowing for multimapping reads (specify `countMultiMappingReads = TRUE`), calling the object `fc.multi`. Check the stats.
