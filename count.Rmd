...
The raw reads, in fastq files, have been aligned using Bowtie2. The bam files containing the aligned reads can be found in the **`RNAseq/data directory`** under the **`Course_Matrerials`**.
...
Running featureCounts generates two output file. A summary statistics table (**`MCL1.DJ.featureCounts.summary`**) and a full table of counts (**`MCL1.DJ.featureCounts`**) for each feature (gene in this case).
...
SRA Toolkit.  
* Whole-genome `.bam` files have also been provided in the `data` folder (`MCL1.*.bam`) aligned using the `bowtie2` aligner. Feel free to explore these files in your own time.
